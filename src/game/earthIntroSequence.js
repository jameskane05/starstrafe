...
const OVERLAY_ID = "earth-opening-overlay";

const LINE1 =
  "You arrive to find Earth's capital ship has suffered a stunning defeat. The rogue swarm is now conducting ground operations from the ship's command center.";
const LINE2 = "You know what to do.";
...
export function mountEarthOpeningOverlayBlack() {
...
export async function runEarthIntroTypewriterAndFade(game) {
  const overlay = document.getElementById(OVERLAY_ID);
  if (!overlay) {
    game.gameManager?.setState?.({ earthIntroTextDone: true });
    return;
  }
